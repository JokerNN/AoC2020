inp = """1-8 n: dpwpmhknmnlglhjtrbpx
...
for line in inp.split('\n'):
    m = match.match(line)
    fro = int(m.group('from'))
    to = int(m.group('to'))
    letter = m.group('letter')
    password = m.group('pass')
    c = Counter(password)
    if letter in c and c[letter] >= fro and c[letter] <= to:
        valid_count_p1 += 1

    if password[fro - 1] == letter and password[to - 1] != letter:
        valid_count_p2 += 1
...
print("Part 1. ", valid_count_p1)
print("Part 2. ", valid_count_p2)
